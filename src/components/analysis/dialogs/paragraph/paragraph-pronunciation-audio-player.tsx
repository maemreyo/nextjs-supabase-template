...
interface ParagraphPronunciationAudioPlayerProps {
  paragraph: string;
...
/**
 * Paragraph Pronunciation Audio Player Component
 * Phát âm đoạn văn với các tùy chọn tùy chỉnh
 */
export const ParagraphPronunciationAudioPlayer: React.FC<ParagraphPronunciationAudioPlayerProps> = ({
  paragraph,
  onPronounce,
  autoPlay = false,
  rate = 0.8,
  ...
  useEffect(() => {
    if (autoPlay && paragraph && isSupported && !isPlaying) {
      handlePronounce();
    }
  }, [autoPlay, paragraph, isSupported]);
  ...
  const handlePronounce = useCallback(() => {
    if (!isSupported || !paragraph) return;
    ...
    try {
      ...
      utterance.onerror = (event) => {
        setIsPlaying(false);
        setIsLoading(false);
        setError('Không thể phát âm đoạn văn. Vui lòng thử lại.');
        utteranceRef.current = null;
      };
  ...
  }, [paragraph, rate, pitch, volume, voice, accent, onPronounce, isSupported]);
  ...
  if (compact) {
    return (
      <Button
        ...
        aria-label={isPlaying ? `Dừng phát âm ${paragraph.substring(0, 20)}...` : `Phát âm ${paragraph.substring(0, 20)}...`}
    ...
    <div className={cn('flex flex-col gap-2', className)} data-testid="pronunciation-player">
      <div className="flex items-center gap-2">
        <Button
          ...
          aria-label={isPlaying ? `Dừng phát âm đoạn văn` : `Phát âm đoạn văn`}
...
export default ParagraphPronunciationAudioPlayer;