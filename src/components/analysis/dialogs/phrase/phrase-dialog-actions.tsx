...
/**
 * Phrase Dialog Actions Component
 * Provides action buttons and dropdown menu for phrase analysis dialog
 */
export const PhraseDialogActions: React.FC<PhraseDialogActionsProps> = ({
  ...
  const handleDelete = useCallback(() => {
    if (window.confirm(`Bạn có chắc muốn xóa cụm từ "${analysis.phrase}"?`)) {
  ...
  const handlePractice = useCallback(() => {
    onPractice?.(analysis.phrase);
  }, [analysis, onPractice]);

  // Handle pronunciation
  const handlePronounce = useCallback(() => {
    // This would typically use a text-to-speech API
    const utterance = new SpeechSynthesisUtterance(analysis.phrase);
    utterance.rate = 0.8;
    utterance.pitch = 1;
  ...
  const actionConfig: PhraseDialogActionConfig = useMemo(() => ({
    primary: [
      {
    ...
    dropdown: [
      {
        label: 'Sao chép cụm từ',
        icon: <Copy className="h-4 w-4" />,
        onClick: () => handleCopy(analysis.phrase, 'phrase'),
        disabled: false,
        loading: false,
      },
      {
        label: 'Sao chép nghĩa đen',
        icon: <FileText className="h-4 w-4" />,
        onClick: () => handleCopy(analysis.literalMeaning || '', 'literal'),
        disabled: !analysis.literalMeaning,
        loading: false,
      },
      {
        label: 'Sao chép nghĩa bóng',
        icon: <FileText className="h-4 w-4" />,
        onClick: () => handleCopy(analysis.contextualMeaning || '', 'contextual'),
        disabled: !analysis.contextualMeaning,
        loading: false,
      },
      {
        label: 'Sao chép bản dịch',
        icon: <FileText className="h-4 w-4" />,
        onClick: () => handleCopy(analysis.vietnameseTranslation || '', 'translation'),
        disabled: !analysis.vietnameseTranslation,
        loading: false,
      },
      {
  ...
  if (compact) {
    ...
    const visiblePrimaryActions = actionConfig.primary.filter((action: any) => !action.hidden);
      ...
      <div className={cn('flex items-center gap-2', className)}>
        ...
        <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            {actionConfig.dropdown.map((action: any, index: number) => (
              <DropdownMenuItem
                key={index}
  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <div className="flex flex-wrap gap-2">
        {actionConfig.primary.filter((action: any) => !action.hidden).map((action: any, index: number) => (
          <Button
            key={index}
            variant="default"
            onClick={action.onClick}
      ...
      {/* Secondary Actions */}
      <div className="flex flex-wrap gap-2">
        {actionConfig.secondary.map((action: any, index: number) => (
          <Button
            key={index}
            variant="outline"
            onClick={action.onClick}
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {actionConfig.dropdown.slice(0, 2).map((action: any, index: number) => (
            <Button
              key={index}
              variant="ghost"
              size="sm"
            ...
            >
              ...
              {copiedText === (index === 0 ? 'phrase' : 'literal') ? (
...
export default PhraseDialogActions;