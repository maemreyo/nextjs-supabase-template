...
/**
 * Hook để tải dữ liệu chi tiết của một session (Refactored version)
 * Bao gồm thông tin session, danh sách analyses, settings và tags
 * Sử dụng API client mới để giảm code duplication
 */
export function useSessionDataRefactored(sessionId: string | undefined, options: UseSessionDataOptions = {}) {
  const {
    enabled = true,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      console.log('🔍 [DEBUG] useSessionDataRefactored - Fetching session data', { sessionId });
      ...
      try {
        ...
        console.log('🔍 [DEBUG] useSessionDataRefactored - Fetch successful', {
          sessionId,
        ...
        console.error('🔍 [DEBUG] useSessionDataRefactored - Fetch failed', error);
  ...
  const invalidateCache = () => {
    console.log('🔍 [DEBUG] useSessionDataRefactored - Invalidating cache');
